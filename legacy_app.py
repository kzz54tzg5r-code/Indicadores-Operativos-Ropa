...
def _read_cache_slice(cache_key, mtime, start_iso="", end_iso=""):
    """Lee un rango de fechas sin cargar el caché completo en memoria.

    V45: el límite final es inclusivo por día y, si el filtro nativo de
    Parquet no es compatible con el tipo de ``Fecha``, se recorre el archivo
    por lotes. Esto evita el fallback anterior que abría 1.3M+ filas y podía
    reiniciar el worker de Streamlit Cloud.
    """
    paths = cache_paths()
    parquet_path = paths[cache_key]
    pickle_path = parquet_path.with_suffix(".pkl")
    start = pd.Timestamp(start_iso).normalize() if start_iso else None
    end = pd.Timestamp(end_iso).normalize() if end_iso else None
    # El usuario filtra por fecha, no por hora. El final debe incluir todo el día.
    ...
    if parquet_path.exists():
        ...
        if end_exclusive is not None:
            filters.append(("Fecha", "<", end_exclusive.to_pydatetime()))
        try:
            frame = pd.read_parquet(parquet_path, filters=filters or None)
            return _filter_frame(frame)
        except Exception:
            # Parquet antiguos pueden guardar Fecha como texto/object. En vez de
            # abrir el archivo completo, leer por lotes y conservar solo coincidencias.
            try:
                ...
                for batch in pf.iter_batches(batch_size=100_000):
                    ...
                    chunk = _filter_frame(chunk)
                    if chunk is not None and not chunk.empty:
                        chunks.append(chunk)
                return pd.concat(chunks, ignore_index=True) if chunks else pd.DataFrame()
            except Exception:
                # Último respaldo seguro: solo para archivos pequeños. Nunca abrir
                # de golpe un caché comercial grande durante un reporte.
                try:
                    if parquet_path.stat().st_size <= 120 * 1024 * 1024:
                        return _filter_frame(pd.read_parquet(parquet_path))
                except Exception:
                    pass
                return pd.DataFrame()

    if pickle_path.exists():
        # Pickle no permite push-down; leer y filtrar. Se conserva por compatibilidad.
        try:
            return _filter_frame(pd.read_pickle(pickle_path))
        except Exception:
            return pd.DataFrame()
    return pd.DataFrame()
...
def load_data_for_page(page, mtime):
    ...
    def _window(latest_value, source, page_name):
        ...
        if page_name == "Operación Diaria":
            raw = st.session_state.get("v25_daily_date")
            try:
    ...
    if page in pages_with_op:
        ...
        # V45: si un rango acotado no devuelve filas, conservar vacío. No abrir
        # todo el histórico como fallback porque puede agotar la memoria.
        if (op is None or op.empty) and op_start is None and op_end is None and op_max is not None:
    ...
    if page in pages_with_co:
        ...
        if (co is None or co.empty) and co_start is None and co_end is None and co_max is not None:
...
def nav_bar():
    ...
    default_page = "Mi Tienda Comercial" if active_app == "Ventas y Análisis Comercial" else "Centro Ejecutivo"
...
''', unsafe_allow_html=True)
st.caption("PS Operaciones Ropa · V64 · Planeación Comercial Macro → Micro")
